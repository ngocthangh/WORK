...
class YelpSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # for i in range(500001, 716200):
        with open('reviewtest1.csv') as csvfile:
            cities = csv.reader(csvfile, quotechar='"', delimiter=',',
                         quoting=csv.QUOTE_ALL, skipinitialspace=True)
            for city in cities:
                yield FormRequest('https://www.agoda.com/pages/agoda/default/DestinationSearchResult.aspx?city=%s' %city[1], callback=self.parse1)
    ...
    def parse1(self, response):
        ...
        for hotel in Hotels:
            ...
            if len(hotelId) > 0:
                ...
                payload = {"hotelId": str(hotelId),"page":'1',"sorting":'1',"filters":{"language":[],"room":[]}}
                yield FormRequest('https://www.agoda.com/NewSite/en-us/Review/ReviewComments', formdata = payload, callback=self.parseCommentNum, method='POST', meta = {'hotelId':hotelId,'DetailLink': DetailLink})
        next = response.css('button#paginationNext')
        if len(next) > 0:
                ...
                for i in range(0, len(nums)):
                    if int(nums[i]) > 0:
                        ...
                        for i in range(1, pageNumber + 1):
                            # if i > 3:
                            #     break
                            payload = {"hotelId": str(hotelId),"page":str(i),"sorting":'1',"filters":{"language":[],"room":[]}}
                            yield FormRequest('https://www.agoda.com/NewSite/en-us/Review/ReviewComments', formdata = payload, callback=self.parseComment, method='POST', dont_filter=True, meta = {'hotelId':hotelId, 'page':i,'DetailLink': DetailLink})
                        break
